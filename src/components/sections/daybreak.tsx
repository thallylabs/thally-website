...
export function Daybreak() {
  ...
  // Template IX2 spans the whole section from enter to leave; the card
  // holds small until 21% and finishes growing at 30%, right at the pin.
  const { scrollYProgress } = useScroll({ target: ref, offset: ["start end", "end start"] });

  // Template smoothing: 95 -> a heavily damped spring trailing the scroll.
  const smooth = useSpring(scrollYProgress, { stiffness: 55, damping: 22, mass: 0.6 });

  const width = useTransform(smooth, [0.21, 0.3], reduced ? ["100%", "100%"] : ["34%", "100%"]);
  const height = useTransform(smooth, [0.21, 0.3], reduced ? ["100vh", "100vh"] : ["44vh", "100vh"]);
  const captionOpacity = useTransform(smooth, [0.24, 0.31], reduced ? [1, 1] : [1, 0]);

  return (
    <section ref={ref} className="bg-canvas relative">
      {/* Pinned stage: card + caption. The card grows into the backdrop. */}
      <div className="sticky top-0 flex h-screen flex-col items-center justify-center gap-8 overflow-hidden">
        <motion.div
          style={{ width, height }}
        ...
        <motion.div style={{ opacity: captionOpacity }} className="px-6 text-center">
