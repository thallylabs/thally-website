...
/**
 * The first two stages use the Thally-labeled diagrams. The sources are 2000px
 * PNGs; the variants beside them are generated by
 * scripts/generate-image-variants.mjs and committed.
 */
const STAGE_DIAGRAMS: Record<number, { base: string; width: number; height: number }> = {
  0: { base: "/template/workflow-diagram", width: 2000, height: 1750 },
  1: { base: "/template/impact-analysis-diagram", width: 2000, height: 1800 },
};

const DIAGRAM_SIZES = "(max-width: 560px) 100vw, 520px";

function StageVisual({ index }: { index: number }) {
  const diagram = STAGE_DIAGRAMS[index];
  if (diagram) {
    return (
      /* display:contents keeps the <img> the direct flex item it was before. */
      <picture className="contents">
        <source
          type="image/avif"
          srcSet={`${diagram.base}-520.avif 520w, ${diagram.base}-1040.avif 1040w`}
          sizes={DIAGRAM_SIZES}
        />
        <source
          type="image/webp"
          srcSet={`${diagram.base}-520.webp 520w, ${diagram.base}-1040.webp 1040w`}
          sizes={DIAGRAM_SIZES}
        />
        <img
          src={`${diagram.base}-520.webp`}
          alt=""
          aria-hidden
          width={diagram.width}
