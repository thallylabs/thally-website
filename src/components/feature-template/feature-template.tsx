...
function TrustedMarquee() {
  return (
    <div className="logos-marquee flex min-w-0 overflow-hidden mask-[linear-gradient(to_right,transparent,black_18%,black_82%,transparent)]">
        ...
        >
          ...
          >
            ...
            {showTrusted && layout !== "offset" && (
              <Reveal delay={0.65} distance={16} className="w-full max-w-[588px] min-w-0">
          ...
          {(layout === "split" || layout === "reverse" || layout === "bleed") && children && (
            <div
              ...
              className={cn("min-w-0", layout === "reverse" && "lg:order-first", layout === "bleed" && "lg:-mr-[12vw]")}
            >
              <motion.div
...
function BoardCardView({ card }: { card: BoardCard }) {
  const base = "rounded-[10px] border border-white/[0.09] bg-[#0f1216]/95 p-3";

  if (card.kind === "filler") {
    return (
      <div
        className="flex flex-col rounded-[10px] border border-white/[0.06] bg-[#0f1216]/70 p-3.5"
        style={{ height: card.h ?? 160 }}
      >
        <div className="flex items-center justify-between border-b border-white/[0.06] pb-2 text-[10px] text-white/35">
          <span>Recent checks</span>
          <span className="font-mono">2m ago</span>
        </div>
        <div className="mt-2.5 space-y-2 text-[10px] text-white/45">
          <p className="flex items-center justify-between gap-3">
            <span className="truncate">Evidence captured</span>
            <span className="shrink-0 font-mono text-white/60">6f3a91c</span>
          </p>
          <p className="flex items-center justify-between gap-3">
            <span className="truncate">Affected paths</span>
            <span className="shrink-0 text-white/60 tabular-nums">2</span>
          </p>
          <p className="flex items-center justify-between gap-3">
            <span className="truncate">Source freshness</span>
            <span className="text-canvas-accent shrink-0">current</span>
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className={base}>
        <div className="flex items-center justify-between text-[10px] text-white/45">
          <span>{card.label ?? "Coverage"}</span>
          <span>{card.progress}%</span>
        </div>
        {card.detail && <p className="mt-2 text-[10px] text-white/40">{card.detail}</p>}
...
export function BannerBoard({
  ...
  if (imageSrc) {
    return (
      <div
        className={cn(
          "overflow-hidden bg-[#07090d]",
          dense ? "aspect-[1.08] max-h-[560px]" : "aspect-[1.72] max-h-[680px]",
        )}
      >
        <img
          src={imageSrc}
  ...
  return (
    <div className={cn("overflow-hidden bg-[#07090d]", dense ? "max-h-[520px]" : "max-h-[640px]")}>
      <div className="flex min-h-12 items-center gap-3 border-b border-white/[0.08] bg-[#0b0e13] px-4">
        ...
        <span className="ml-auto hidden items-center gap-1.5 text-[10px] text-white/40 sm:flex">
          <span className="bg-canvas-accent size-1.5 rounded-full" />
          Synced 2m ago
        </span>
      </div>

      <div className={cn(!dense && "lg:grid lg:grid-cols-[164px_minmax(0,1fr)]")}>
        {!dense && (
          <aside className="hidden border-r border-white/[0.07] bg-[#090c10] px-3 py-5 lg:block">
            <p className="px-2 text-[10px] font-medium tracking-[0.14em] text-white/25 uppercase">Workspace</p>
            <nav className="mt-3 space-y-1 text-[11px] text-white/42" aria-label="Product preview">
              {(
                [
                  ["Overview", false],
                  ["Changes", true],
                ...
                <span
                  ...
                  className={cn(
                    ...
                    active && "bg-white/[0.07] text-white/80",
                  )}
                >
                  {item}
                  {item === "Drafts" && <span className="rounded-full bg-white/10 px-1.5 text-[9px]">3</span>}
                </span>
              ))}
            </nav>
            <div className="mt-6 border-t border-white/[0.06] px-2 pt-4 text-[10px] text-white/30">
              <p>northstar-labs/docs</p>
              <p className="mt-1 font-mono">main · 6f3a91c</p>
            </div>
          </aside>
        )}
